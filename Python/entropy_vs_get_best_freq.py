...
def pick_better_freq(word_list, player_words=None, word_length=5):
    if player_words is None:
...
def get_enthropy(words, word, word_size=5):
    ...
    for pattern in combinations:
        ...
        P = len(words_in_pattern) / len(words)
        E += P * math.log(1 / P, 2)

    return E


def get_best_enthropy(words, player_words=None):
    if player_words is None:
        player_words = words
    best_rat = 0
    best_word = words[0]
...
def get_tree(words, word, player_words=None, word_size=5):
    if player_words is None:
        player_words = words
    pattern_words = {}
    ...
    for pattern in combinations:
        ...
        if len(words_in_pattern) == 1:
            ...
            best_word = get_best_enthropy(words_in_pattern, player_words=player_words)
            pattern_words[str(pattern)][ex_toString(best_word)] = get_tree(words_in_pattern, best_word)

    return pattern_words


def get_tree_better_freq(words, word, player_words=None, word_size=5):
    if player_words is None:
        player_words = words
    pattern_words = {}
...
def save_get_tree_better_freq(words, word_start):
    res2 = get_tree_better_freq(words, word_start, player_words=get_words()[:])
    word_trees_2 = {}
...
def save_trees():
    ...
    words = get_words()[:]
    # count = 0
    # for x, f in get_frequency().items():
    #     print(x, f)
    #     count += 1
    #     if count == 100:
    #         break
    # print(get_frequency()[:100])
...
# wl = [['m', 'e', 'n', 't', 'a'], ['ż', 'e', 'n', 't', 'a'], ['p', 'o', 'n', 't', 'a']]
# res = get_enthropy(wl, expand_word("tantx"))
# print(res)
